...
mod ts_hybrid_cross {
    ...
    fn rust_verifies_ts_produced_hybrid_signature() {
        ...
        let sig: Vec<u8> = (0..6746).step_by(2)
            .map(|i| u8::from_str_radix(&TS_HYBRID_HEX[i..i+2], 16).unwrap()).collect();
        assert_eq!(sig.len(), 64 + ML_DSA_SIG_LEN);
        let mut ed = [0u8; 64];
        ed.copy_from_slice(&sig[..64]);
        assert!(crate::verify(&ed_pk, &msg, &ed).is_ok(), "ed25519 half (TS->Rust) must verify");
        assert!(verify_ml_dsa(&ml_pk, &msg, &sig[64..]).is_ok(), "ml-dsa half (TS->Rust) must verify");
    }
